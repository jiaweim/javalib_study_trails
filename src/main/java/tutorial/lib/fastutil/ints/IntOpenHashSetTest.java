...
//
//		if ( s.containsNull && ! s.contains( 0 ) ) throw new AssertionError( "Hash table should contain zero by
// internal state, but it doesn't when queried" );
//		if ( ! s.containsNull && s.contains( 0 ) ) throw new AssertionError( "Hash table should not contain zero by
// internal state, but it does when queried" );
//
//		java.util.HashSet<Integer> t = new java.util.HashSet<Integer>();
//		for ( int i = s.size(); i-- != 0; )
//			if ( key[ i ] != 0 && !t.add( key[ i ] ) ) throw new AssertionError( "Key " + key[ i ] + " appears
// twice" );
//
//	}
//
...
//				) + "; actual: " + expected + "; stddev: " + Math.sqrt( totSquareProbes / m.n - expected * expected )
...
//		}
//
//		assertEquals( "Error: m has only " + c + " keys instead of " + t.size() + " after insertion (iterating on m)",
// c, t.size() );
//		/*
...
//		for ( int i = 0; i < n; i++ ) {
//			int T = genKey();
//			assertEquals( "Error: divergence in keys between t and m (polymorphic method)", m.contains( T ), t
// .contains( ( Integer.valueOf( T ) ) ) );
//		}
//
//		/*
...
//		for ( int i = 0; i < n; i++ ) {
//			int T = genKey();
//			assertFalse( "Error: divergence between t and m (standard method)", m.contains( ( Integer.valueOf( T ) ) )
...
//			int T = genKey();
//			assertFalse( "Error: divergence in add() between t and m", m.add( ( Integer.valueOf( T ) ) ) != t.add( (
// Integer.valueOf( T ) ) ) );
//			T = genKey();
//			assertFalse( "Error: divergence in remove() between t and m", m.remove( ( Integer.valueOf( T ) ) ) != t
// .remove( ( Integer.valueOf( T ) ) ) );
//		}
//
//		checkTable( m );
...
//		int a[] = m.toIntArray();
//
//		assertTrue( "Error: toArray() output (or array-based constructor) is not OK", new IntOpenHashSet( a ).equals(
// m ) );
//
...
//		for ( int i = 0; i < 20 * n; i++ ) {
//			int T = genKey();
//			assertFalse( "Error: divergence in add() between t and m after save/read", m.add( ( Integer.valueOf( T ) )
// ) != t.add( ( Integer.valueOf( T ) ) ) );
//			T = genKey();
//			assertFalse( "Error: divergence in remove() between t and m after save/read", m.remove( ( Integer.valueOf(
// T ) ) ) != t.remove( ( Integer.valueOf( T ) ) ) );
//		}
//
...
//
//		for( int i = n; i-- != 0; )
//			assertEquals( "Error: m and t differ on a key during torture-test insertion.", m.add( i ), t.add( (
// Integer.valueOf( i ) ) ) );
//
...
//
//		for( int i = n; i-- != 0; )
//			assertEquals( "Error: m and t differ on a key during torture-test insertion.", m.remove( i ), t.remove( (
// Integer.valueOf( i ) ) ) );
//
