...
public class IntBigArrayBigListTest
{
    ...
    protected static void testLists(IntBigList m, IntBigList t, int n, int level)
    {
        ...
        for (int i = 0; i < 10 * n; i++) {
            int T = genKey();
            assertEquals(t.indexOf((Integer
                    .valueOf(T))), m.indexOf((Integer.valueOf(T))));
            assertEquals(t.lastIndexOf((Integer.valueOf(T))), m.lastIndexOf((Integer.valueOf
                    (T))));
            assertEquals(t.indexOf((Integer.valueOf(T))), m.indexOf(T));
            assertEquals(t.lastIndexOf((Integer
                    .valueOf(T))), m.lastIndexOf(T));
        }
        /* Now we check cloning. */
        if (level == 0) {
            assertEquals(((IntBigArrayBigList) m).clone(), m);
            assertEquals(m, ((IntBigArrayBigList) m).clone());
        }
        /* Now we play with constructors. */
        assertEquals(new IntBigArrayBigList((IntCollection) m), m);
        assertEquals(m, (new IntBigArrayBigList((IntCollection) m)));
        assertEquals(new IntBigArrayBigList(m), m);
        assertEquals(m, (new IntBigArrayBigList(m)));
        assertEquals(new IntBigArrayBigList(m.listIterator()), m);
        assertEquals(m, (new IntBigArrayBigList(m.listIterator())));
        assertEquals(new IntBigArrayBigList(m.iterator()), m);
        assertEquals(m, (new IntBigArrayBigList(m.iterator())));
        int h = m.hashCode();
