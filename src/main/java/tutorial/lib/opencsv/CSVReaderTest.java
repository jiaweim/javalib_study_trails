...
public class CSVReaderTest
{
    ...
    public void parseQuotedStringWithDefinedSeperator() throws IOException
    {
        StringBuilder sb = new StringBuilder(CSVParser.INITIAL_READ_SIZE);
        sb.append("a\tb\tc").append("\n"); // tab separated case
        CSVReader c = new CSVReaderBuilder(new StringReader(sb.toString())).withCSVParser(new CSVParserBuilder().withSeparator('\t').build()).build();
    ...
    public void bug106ParseLineWithCarriageReturnNewLineStrictQuotes() throws IOException
    {
        ...
        CSVReader c = new CSVReader(new StringReader(sb.toString()), CSVParser.DEFAULT_SEPARATOR,
                CSVParser.DEFAULT_QUOTE_CHARACTER, CSVParser.DEFAULT_ESCAPE_CHARACTER, CSVReader.DEFAULT_SKIP_LINES,
                true, CSVParser.DEFAULT_IGNORE_LEADING_WHITESPACE, true);
    ...
    public void creatingIteratorForReaderWithNullDataThrowsRuntimeException() throws IOException
    {
        try {
            ...
            when(mockReader.read(ArgumentMatchers.<CharBuffer>any())).thenThrow(new IOException("test io exception"));
            ...
            when(mockReader.read((char[]) notNull(), anyInt(), anyInt())).thenThrow(
