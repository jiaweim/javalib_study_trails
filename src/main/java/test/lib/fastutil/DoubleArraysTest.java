...
import test.lib.fastutil.ints.IntArraysTest;
...
public class DoubleArraysTest
{
    ...
    void testRadixSort2()
    {
        ...
        for (int i = d[0].length - 1; i-- != 0; )
            assertTrue(d[0][i] < d[0][i + 1] || d[0][i] == d[0][i + 1] && d[1][i] <= d[1][i + 1],
                    Integer.toString(i) + ": <" + d[0][i] + ", " + d[1][i] + ">, <" + d[0][i + 1] + ", " + d[1][i
                            + 1] + ">");
    ...
    void testRadixSortIndirectStable()
    {
        ...
        for (int i = 10; i < 89; i++) assertTrue(d[perm[i]] <= d[perm[i + 1]]);
    ...
    void testRadixSort2IndirectStable()
    {
        ...
        for (int i = t.length - 1; i-- != 0; )
            assertTrue(t[perm[i]] < t[perm[i + 1]] || t[perm[i]] == t[perm[i + 1]] && u[perm[i]] <= u[perm[i + 1]]);
        ...
        DoubleArrays.radixSortIndirect(perm, t, u, true);
        for (int i = t.length - 1; i-- != 0; )
            if (t[perm[i]] == t[perm[i + 1]] && u[perm[i]] == u[perm[i + 1]]) assertTrue(perm[i] < perm[i + 1]);
        ...
        for (int i = 10; i < 89; i++) assertTrue(u[perm[i]] <= u[perm[i + 1]]);
        for (int i = 0; i < 10; i++) assertEquals(i, perm[i]);
        for (int i = 90; i < 100; i++) assertEquals(i, perm[i]);
    ...
    void testRadixSort2IndirectUnstable()
    {
        ...
        DoubleArrays.radixSortIndirect(perm, t, u, false);
        for (int i = t.length - 1; i-- != 0; )
            assertTrue(t[perm[i]] < t[perm[i + 1]] || t[perm[i]] == t[perm[i + 1]] &&
                    u[perm[i]] <= u[perm[i + 1]]);
    }
    ...
    @Test
    void testRadixSortIndirect2NaNs()
    {
        final double[] t = {Double.NaN, 1, 5, 2, 1, 0, 9, 1, Double.NaN, 2, 4, 6, 8, 9, 10, 12, 1, 7};
        for (int to = 1; to < t.length; to++)
            for (int from = 0; from < to; from++) {
