...
public class InspectableFileCachedInputStreamTest
{
    ...
    static {
        ...
        byte[] b;
        // Now generates byte buffers from 1 byte up to 64KiB; we shuffle them so that they are not increasing in
        // size...
        for (int k = 0; k < 10; k++) {
            b = new byte[1 << k];
            r.nextBytes(b);
    ...
    {
        final InspectableFileCachedInputStream icis = new InspectableFileCachedInputStream(4, File.createTempFile
                (getClass().getSimpleName(), "overflow"));
    ...
    public void testClosed() throws IOException
    {
        ...
        assertThrows(IOException.class, () -> icis.read());
    }

    @Test
    public void testDisposed() throws IOException
    {
        @SuppressWarnings("resource") final InspectableFileCachedInputStream icis = new InspectableFileCachedInputStream(4);
        final byte[] data = new byte[]{1, 2};
        icis.write(ByteBuffer.wrap(data));
        icis.dispose();
        assertFalse(icis.isOpen());
    ...
    {
        @SuppressWarnings("resource") final InspectableFileCachedInputStream icis = new InspectableFileCachedInputStream();
        final byte[] data = new byte[]{1, 2};
        icis.write(ByteBuffer.wrap(data));
        icis.dispose();
        assertThrows(IOException.class, () -> icis.clear());
    ...
    public void testResetDisposed() throws IOException
    {
        ...
        icis.write(ByteBuffer.wrap(data));
        icis.dispose();
        assertThrows(IOException.class, () -> icis.reset());
    ...
    {
        assertThrows(IllegalArgumentException.class, () -> new InspectableFileCachedInputStream(-1));
