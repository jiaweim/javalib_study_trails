...
public class Int2IntOpenHashMapTest {
    ...
    protected static void test(int n, float f) throws IOException, ClassNotFoundException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            assertTrue(
                    !(m.get(T) != (0)) != ((t.get((Integer.valueOf(T))) == null ? (0) : ((((Integer) (t.get((Integer
                            .valueOf(T))))).intValue()))) != (0)) ||
        ...
        for (int i = 0; i < 20 * n; i++) {
            ...
            int U = genValue();
            assertTrue(
                    valEquals(m.put((Integer.valueOf(T)), (Integer.valueOf(U))), t.put((Integer.valueOf(T)), (Integer
                            .valueOf(U)))));
            T = genKey();
        ...
        for (java.util.Iterator i = t.entrySet().iterator(); i.hasNext(); ) {
            Map.Entry e = (Map.Entry) i.next();
            assertTrue(valEquals(e
                    .getValue(), m.get(e.getKey())));
        }
		/* Now we check that m actually holds that data, but iterating on m. */
        for (java.util.Iterator i = m.entrySet().iterator(); i.hasNext(); ) {
            Map.Entry e = (Map.Entry) i.next();
            assertTrue(valEquals(e
                    .getValue(), t.get(e.getKey())));
        }
		/* Now we check that m actually holds the same keym. */
        for (java.util.Iterator i = t.keySet().iterator(); i.hasNext(); ) {
            Object o = i.next();
        ...
        for (int i = 0; i < 20 * n; i++) {
            ...
            T = genKey();
            Integer result;
            assertTrue(valEquals(m.remove(T),
                    (result = (Integer) t.remove((Integer.valueOf(T)))) != null ? result.intValue() : 0));
        }
        assertTrue(m.equals(t));
        assertTrue(t.equals(m));
		/*
        ...
        t.putAll(m);
        for (int i = n; i-- != 0; )
            assertEquals(m.put(i, 2), t.put(i, 2));

        assertTrue(m.equals(t));
        assertTrue(t.equals(m));
        assertTrue(m.equals(m.clone()));
        assertTrue(m.clone().equals(m));
        m.trim();
        assertTrue(m.equals(t));
        assertTrue(t.equals(m));
        return;
...
//        assertEquals(8, m.n); // current table size.
